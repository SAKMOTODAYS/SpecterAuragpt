...
// Each array = charCode XOR _m for each character of the original key
// To add a new key: [...'YOUR_KEY'].map(c=>c.charCodeAt(0)^_m)
const _m = 99;
const _v = [
...
function saveProfile(p)  { localStorage.setItem("aura-profile", JSON.stringify(p)); }
function loadProfile()   {
  try { return JSON.parse(localStorage.getItem("aura-profile")); }
  catch { return null; }
}
function clearProfile()  { localStorage.removeItem("aura-profile"); }
...
//  ENTER CHAT
// ══════════════════════════════════════════════════════════════════════════════
function enterChat(user, profile) {
...
//  كل key كيتحجب نحفظو مع timestamp — ما نعاودوش عليه حتى يعدى 62 ثانية
// ══════════════════════════════════════════════════════════════════════════════
const _cooldown   = {}; // "keyIdx:modelIdx" → timestamp (ms)
const COOLDOWN_MS = 62_000; // 62s > الـ 60s window ديال Gemini
...
// أرجع أول combo ماشي على cooldown، ولا null إلا كلهم محجوبين
...
// وقت الانتظار حتى يتحرر أقرب combo
function _msUntilNext() {
  ...
  return Math.max(0, earliest) + 500; // +500ms هامش
}

// ══════════════════════════════════════════════════════════════════════════════
//  RETRY ENGINE — cooldown-aware, no wasted requests
// ══════════════════════════════════════════════════════════════════════════════
async function fetchWithRetry(typing) {
  // خطأ مؤقت → نعاود على key آخر
  const isRateErr = err =>
    err.status === 429 ||
    err.status === 503 ||
  ...
  // كوتا اليوم خلصت لهاد الـ key → نجربو على key آخر بس ما ننتظروش
  ...
  while (Date.now() < deadline) {
    ...
    if (!combo) {
      if (allDailyExhausted) {
        // كل الـ keys خلصات كوتا اليوم
        const e = new Error("DAILY_QUOTA_ALL");
        e.dailyExhausted = true;
        throw e;
      }
      // كل الـ keys على cooldown مؤقت — انتظر حتى يتحرر الأقرب
    ...
    try {
      ...
      if (isDailyQuota(err)) {
        // هاد الـ key خلصت كوتا اليوم — علّم عليه بـ cooldown طويل (23 ساعة)
        _cooldown[_comboKey(combo.k, combo.m)] = Date.now() - COOLDOWN_MS + 23 * 60 * 60_000;
        // شوف واش كل الـ keys وصلوا لهاد الحال
        allDailyExhausted = _nextAvailable() === null;
        continue;
      }
      if (isRateErr(err)) {
        // rate limit مؤقت — cooldown عادي 62 ثانية
        _markCooling(combo.k, combo.m);
        allDailyExhausted = false;
        continue;
      }
      // خطأ آخر (شبكة، key غير صالح...)
      throw err;
    }
  }
...
function errMsg(err) {
  ...
  // Debug: show real error (remove this line in production)
